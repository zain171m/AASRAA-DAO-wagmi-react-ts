  ...
  return (
    ...
    >
      <div className="py-3 flex flex-col gap-2">
        <h1 className="text-2xl text-white">All Unapproved Campaigns</h1>
        <p className="text-sm text-zinc-300">
          Discover all Unapproved campaings and vote for their approval"
        </p>
      </div>
      ...
      <div className=" grid lg:grid-cols-3 sm:grid-cols-2 xs:sm:grid-cols-1 gap-4">
        ...
        {result.isFetched &&
          campaigns?.map((campaign, i) => (
            <Link to={`/details/${i}`} state={{ campaign }} key={i}>
              <div
                key={i}
